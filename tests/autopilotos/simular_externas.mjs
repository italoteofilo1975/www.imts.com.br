...
// pré-voo: recusa validar o site errado (ex.: domínio ainda apontando para site legado)
{
  ...
  if (!(conectar.status === 200 && /briefing/i.test(conectar.html) && gateway.status === 200 && gateway.html.includes("imts-site-integration-gateway"))) {
    console.error(`⚠️  PRÉ-VIO FALHOU: ${BASE} não está servindo o app IMTS (/conectar ${conectar.status} · gateway ${gateway.status}). Exit 3.`);
    process.exit(3);
  }
  console.log("✓ pré-vio: app IMTS confirmado");
}
