type StoredLead={id:string;intent:string;name:string;email:string;organization:string;role:string;message:string;destination:string;createdAt:string;privacy:{consentVersion:string}};
...
async function ensureSchema(db:D1Database){
  await db.batch([
    db.prepare("CREATE TABLE IF NOT EXISTS leads (id TEXT PRIMARY KEY, intent TEXT NOT NULL, name TEXT NOT NULL, email TEXT NOT NULL, organization TEXT NOT NULL DEFAULT '', role TEXT NOT NULL DEFAULT '', message TEXT NOT NULL, destination TEXT NOT NULL, consent_version TEXT NOT NULL, delivery_status TEXT NOT NULL DEFAULT 'accepted', delivery_channel TEXT, delivery_attempts INTEGER NOT NULL DEFAULT 0, last_error TEXT, created_at TEXT NOT NULL, updated_at TEXT NOT NULL)"),
